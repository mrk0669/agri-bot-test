...
| | |
|---|---|
| **Base photograph** | AgML *Weed Crop Detection*, image `train-00014-of-00016 #0035` |
...
| | |
|---|---|
| **Photograph** | AgML *Weed Crop Detection*, image `train-00014-of-00016 #0008` |
| **Source / licence / citation** | as above — **CC BY 4.0** |
| **Modification** | Resized to 1500 px on the long side. Otherwise unaltered. |
| **Important** | This image is in the **validation split** — it was held out of training, so the Tier 2 result is on data the model has never seen. |
...
python tools/make_demo_scene.py                           # Tier 1 composite
python tools/demo_three_tiers.py                          # all three panels
```
...
|---|---|---|---|
| 1 Colour + geometry | nothing | 4/4 markers found; red cloth rejected on `aspect 3.26 > 3.00` | 46 ms CPU |
| 2 YOLO11n fine-tuned | 287 labelled images | 2 weeds, 5 crops on a held-out image | 39 ms |
| 3 YOLO-World v2-XL | nothing but text | 2 regions, best confidence 0.42 | 67 ms |
